...
export class C9Company implements CompanyScraper {
  company = "Cloud9";
  mainUrl = "https://apply.workable.com/cloud9/j/";
  ...
  async scrapeLinks() {
    ...
    try {
      const response = await axios.post(
        "https://apply.workable.com/api/v3/accounts/cloud9/jobs",
        {
          department: [],
          location: [],
          query: "",
  ...
  async scrapeJobOffer(url: string): Promise<JobOffer> {
    try {
      const response = await axios.get(
        "https://apply.workable.com/api/v2/accounts/cloud9/jobs/" + url
      );
